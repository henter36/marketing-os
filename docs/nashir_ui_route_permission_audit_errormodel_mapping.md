...
| headline change | yes | yes | Approval State Machine Contract | `NUS-CONTENT-001`, `NUS-REAPPROVAL-001` | Required. | Exact headline field mapping needed. |
| CTA change | yes | yes | Approval State Machine Contract | `NUS-CAMPAIGN-001`, `NUS-REAPPROVAL-001` | Required. | Exact CTA field mapping needed. |
| Offer/CTA change | yes | yes | Approval State Machine Contract, Readiness Scoring Contract | `NUS-OBJECT-001`, `NUS-CAMPAIGN-001`, `NUS-REAPPROVAL-001` | Required. | Offer terms taxonomy needed. |
| landing destination change | yes | yes | Approval State Machine Contract | `NUS-DESTINATION-001`, `NUS-UTM-001`, `NUS-REAPPROVAL-001` | Required. | Destination normalization needed. |
| image/video/asset change | yes | yes | Approval State Machine Contract | `NUS-RIGHTS-001`, `NUS-VIDEO-001`, `NUS-REAPPROVAL-001` | Required. | Asset version/hash mapping needed. |
| hashtags change | yes | yes, unless future QA defines safe non-material criteria | Approval State Machine Contract | `NUS-HASHTAGS-001`, `NUS-REAPPROVAL-001` | Required if approved content changed. | Channel/hash materiality policy needed. |
| channel change | yes | yes | Approval State Machine Contract | `NUS-CAMPAIGN-001`, `NUS-HASHTAGS-001`, `NUS-REAPPROVAL-001` | Required. | Channel vocabulary needed. |
| promotion terms change | yes | yes | Approval State Machine Contract, Readiness Scoring Contract | `NUS-OBJECT-001`, `NUS-CAMPAIGN-001`, `NUS-REAPPROVAL-001` | Required. | Promotion term fields needed. |
| claims/risk wording change | yes | yes | Approval State Machine Contract, Threat Model | `NUS-CONTENT-001`, `NUS-RIGHTS-001`, `NUS-REAPPROVAL-001` | Required. | Claims policy categories needed. |
| creative rights status change | yes | yes | Approval State Machine Contract | `NUS-RIGHTS-001`, `NUS-REAPPROVAL-001` | Required. | Rights state vocabulary needed. |
| UTM link change when material to destination or campaign | yes | yes | Approval State Machine Contract, UTM Lite planning | `NUS-DESTINATION-001`, `NUS-UTM-001`, `NUS-REAPPROVAL-001` | Required. | UTM materiality criteria needed. |
...
| Audit event expectation | `nashir.manual_checklist.completed` if persisted. |
...
| Audit event expectation | `nashir.evidence.submitted` and `nashir.evidence.reviewed`. |
| Readiness status | Needs clarification; not implementation-ready. |
...
| Audit event expectation | `nashir.utm_link.created`; mismatch resolution audit if future policy approves. |
| Readiness status | Needs clarification; not implementation-ready. |
...
| OAuth/social account connection | No connect-account UI in Core V1. | Blocked / forbidden. | Audit if attempted. | External integration bypass. | `NQA-NOGO-001` | Blocked. |
| scheduling | No schedule controls. | Blocked / forbidden. | Audit if attempted. | Publishing/scheduling creep. | `NQA-NOGO-001` | Blocked. |
| paid ads | No launch/spend/budget execution controls. | Blocked / forbidden. | Audit if attempted. | Paid execution bypass. | `NQA-NOGO-001` | Blocked. |
| payments | No payment, billing, invoice, refund, tax, or provider billing UI. | Blocked / forbidden. | Audit if attempted. | Payment scope creep. | `NQA-NOGO-001` | Blocked. |
| analytics ingestion | No import/connect/sync analytics controls. | Blocked / forbidden. | Audit if attempted. | Analytics ingestion creep. | `NQA-NOGO-001` | Blocked. |
| attribution | Label UTM/manual performance as non-attribution. | Blocked / forbidden if attribution is attempted. | Audit if attempted. | Attribution confusion. | `NQA-NOGO-001`, `NQA-UTM-001` | Blocked. |
| external integrations | No connector, API sync, external tool execution, or account link UI. | Blocked / forbidden. | Audit if attempted. | External integration bypass. | `NQA-NOGO-001` | Blocked. |
| autonomous AI execution | Label AI as advisory-only; no run-agent controls. | Blocked / forbidden. | Audit advisory/protected-action attempt if applicable. | AI suggestion misuse. | `NQA-NOGO-002` | Blocked. |
| Post-V1 module implementation | Label Post-V1 modules as reference-only if mentioned. | Blocked / forbidden. | Audit if attempted in future workflow. | Scope expansion. | `NQA-NOGO-001` | Blocked. |
| readiness-as-approval | Display readiness as advisory and separate from approval. | Error/block if readiness is used as approval. | Audit if attempted. | Approval bypass. | `NQA-READINESS-001` | Blocked. |
| evidence-as-authorization | Display evidence as proof only, not authorization. | Error/block if evidence triggers publishing. | Audit if attempted. | Evidence mistaken as authorization. | `NQA-EVIDENCE-003` | Blocked. |
...
|---|---|---|---|---|---|---|---|---|---|---|
| `NUS-READINESS-001` | `AC-READINESS-001` through `AC-READINESS-004` | `NQA-READINESS-001` through `NQA-READINESS-004` | Readiness Dashboard | `/workspaces/:workspaceId/nashir/readiness` | View permission; readiness grants no authority. | Optional view/recalculation audit if persisted. | Block readiness-as-approval/publishing. | `OnboardingProgress`, `SetupChecklistItem`, `Campaign`, `BriefVersion`, `ApprovalDecision`, `ManualPublishEvidence`, `AuditLog` | Onboarding, campaign, brief-version, approval, manual evidence, audit read surfaces | Needs clarification. |
...
| `NUS-CAMPAIGN-001` | `AC-CAMPAIGN-001`, `AC-CAMPAIGN-002` | `NQA-CAMPAIGN-001`, `NQA-CAMPAIGN-002` | Campaign basics | `/workspaces/:workspaceId/nashir/campaigns/:campaignId/basics` | Draft/edit permission; exact code gap. | Campaign save/edit audit if persisted. | Validation for required campaign fields. | `Campaign`, `CampaignStateTransition`, `BriefVersion` | Campaign, campaign state transition, brief-version paths | Needs clarification. |
| `NUS-DESTINATION-001` | `AC-DESTINATION-001`, `AC-DESTINATION-002` | `NQA-DESTINATION-001`, `NQA-DESTINATION-002` | Landing destination | `/workspaces/:workspaceId/nashir/campaigns/:campaignId/destination` | Edit/review permission; exact code gap. | Destination material-change audit. | Invalid destination or reapproval required. | `BriefVersion`, `TrackedLink`, `PublishJob`, `MediaAssetVersion`, `ApprovalDecision`, `AuditLog` | Brief-version, tracked-link, asset-version, approval decision paths | Needs clarification. |
...
| `NUS-CONTENT-001` | `AC-CONTENT-001`, `AC-CONTENT-002` | `NQA-CONTENT-001`, `NQA-CONTENT-002` | Ideas/content requirements | `/workspaces/:workspaceId/nashir/campaigns/:campaignId/content` | Draft/edit/review permission; AI advisory-only. | Intake/content saved and AI advisory events if persisted. | Protected-field/AI denial. | `BriefVersion`, `MediaAssetVersion`, `ApprovalDecision`, `AuditLog` | Brief-version, asset-version, approval, audit surfaces | Needs clarification. |
| `NUS-HASHTAGS-001` | `AC-HASHTAGS-001` | `NQA-HASHTAGS-001` | Hashtag drafts | `/workspaces/:workspaceId/nashir/campaigns/:campaignId/content` | Draft/edit/review permission; AI advisory-only. | Audit if submitted or materially changed. | Block optimization/attribution claims. | `BriefVersion`, `MediaAssetVersion` | Brief-version and asset-version paths | Needs clarification. |
| `NUS-VIDEO-001` | `AC-VIDEO-001` | `NQA-VIDEO-001` | Video reference scripts | `/workspaces/:workspaceId/nashir/campaigns/:campaignId/content` | Draft/edit/review permission; AI advisory-only. | Audit if submitted or materially changed. | Block final video production actions. | `BriefVersion`, `MediaAssetVersion` | Brief-version and asset-version paths | Needs clarification. |
| `NUS-UTM-001` | `AC-UTM-001`, `AC-UTM-002` | `NQA-UTM-001`, `NQA-UTM-002` | UTM Tracking Lite | `/workspaces/:workspaceId/nashir/campaigns/:campaignId/utm` | Tracked-link edit/review permission; exact code gap. | `nashir.utm_link.created`. | Block attribution/analytics; mismatch correction. | `TrackedLink`, `PublishJob`, `ManualPublishEvidence`, `AuditLog` | Tracked-link and manual evidence paths | Needs clarification. |
| `NUS-APPROVAL-001` | `AC-APPROVAL-001`, `AC-APPROVAL-002` | `NQA-APPROVAL-001`, `NQA-APPROVAL-002` | Human approval | `/workspaces/:workspaceId/nashir/campaigns/:campaignId/review` | Explicit human approval permission; exact code gap. | `nashir.approval.accepted` / `nashir.approval.rejected`. | Unauthorized/invalid transition errors. | `ReviewTask`, `ApprovalDecision`, `MediaAssetVersion`, `AuditLog` | Review-task and approval-decision paths | Needs clarification. |
...
| `NUS-CHECKLIST-001` | `AC-CHECKLIST-001` | `NQA-CHECKLIST-001` | Manual publishing checklist | `/workspaces/:workspaceId/nashir/campaigns/:campaignId/manual-checklist` | Prepare checklist permission; exact code gap. | `nashir.manual_checklist.completed` if persisted. | Block publish/schedule/spend/connect. | `PublishJob`, `SetupChecklistItem`, `ApprovalDecision`, `ManualPublishEvidence`, `AuditLog` | Publish-job and manual-evidence paths; no clear `SetupChecklistItem` path | Needs clarification; OpenAPI not clear. |
| `NUS-EVIDENCE-001` | `AC-EVIDENCE-001` through `AC-EVIDENCE-004` | `NQA-EVIDENCE-001` through `NQA-EVIDENCE-004` | Manual publishing evidence | `/workspaces/:workspaceId/nashir/campaigns/:campaignId/evidence` | Evidence submit/review permission; exact code gap. | `nashir.evidence.submitted`, `nashir.evidence.reviewed`. | Invalid evidence, unauthorized review, no publishing authorization. | `ManualPublishEvidence`, `PublishJob`, `MediaAssetVersion`, `ApprovalDecision`, `TrackedLink`, `AuditLog` | Manual evidence, publish-job, tracked-link paths | Needs clarification. |
...
| `NUS-ERRORS-001` | `AC-ERRORS-001`, `AC-ERRORS-002` | `NQA-ERRORS-001`, `NQA-ERRORS-002` | ErrorModel/idempotency | All workspace-scoped Nashir surfaces | All human actors; AI denied protected actions. | Error/idempotency conflict audit if applicable. | Preserve ErrorModel and declared idempotency. | `AuditLog`, workspace-scoped entities, `MediaJob`, `PublishJob`, `UsageMeter` | `ErrorModel`, `ErrorResponse`, `Idempotency-Key` where declared | Blocked. |
