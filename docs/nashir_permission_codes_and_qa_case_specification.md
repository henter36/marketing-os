...
| `nashir.approval.request_changes` | Human request for changes during review. | Does not approve; must be explicit, authorized, auditable, and version-bound. |
...
| `NQA-EVIDENCE` | Verify evidence remains proof only. | Review URL/screenshot/version/UTM evidence expectations. | Wrong version evidence; accept/request correction/supersede/invalidate authorization; self-submission denial; AI evidence denial. | Evidence submit/review allowed where authorized; authorization bypass and self-submission acceptance/invalidation denied. | `NUS-EVIDENCE-001` | `AC-EVIDENCE-001` through `AC-EVIDENCE-004` | NO |
