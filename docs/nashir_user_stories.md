...
`docs/nashir_threat_model_update.md` identifies tenant isolation, workspace trust boundary, authorization, approval, evidence, UTM Lite, manual performance, AI assistant, creative rights, claims, audit, and NO-GO bypass threats. User stories must preserve those threat boundaries.
...
12. AI assistant must not approve, reject, accept evidence, invalidate evidence, publish, schedule, spend, connect accounts, ingest analytics, attribute results, or change protected fields.
...
| Persona / actor | Human role or advisory-only AI assistant reference associated with the story. |
...
| Capability | User Story ID | Focus | Primary persona / actor | Planning summary |
|---|---|---|---|---|
...
| Idea intake and content requirements | `NUS-CONTENT-001` | Draft content planning | editor, reviewer, AI assistant | Capture ideas and requirements as draft/advisory inputs pending review. |
| Hashtags per selected channel | `NUS-HASHTAGS-001` | Channel hashtag drafts | editor, reviewer, AI assistant | Produce or manage hashtag draft recommendations without analytics or reach claims. |
| Video reference scripts | `NUS-VIDEO-001` | Reference scripts | editor, reviewer, AI assistant | Prepare script references only, not video production or final generation. |
...
| Role & permission boundaries | `NUS-PERMISSIONS-001` | Protected-action authority | owner, admin, reviewer, evidence reviewer, editor, viewer, AI assistant | Preserve explicit permissions, viewer denial, and AI assistant denial for protected actions. |
...
| Idea intake and content requirements | `NUS-CONTENT-001` | editor, reviewer, AI assistant | `AC-CONTENT-001`, `AC-CONTENT-002` | `NQA-CONTENT-001`, `NQA-CONTENT-002` | `BriefVersion`, `MediaAssetVersion`, `ApprovalDecision`, `AuditLog` | Brief-version, asset-version, approval, audit surfaces | AI assistant suggestions must not change protected fields or bypass review/approval/reapproval. | Planned only; not implemented |
| Hashtags per selected channel | `NUS-HASHTAGS-001` | editor, reviewer, AI assistant | `AC-HASHTAGS-001` | `NQA-HASHTAGS-001` | `BriefVersion`, `MediaAssetVersion` | Brief-version and asset-version paths | No reach guarantee, trend ingestion, analytics optimization, or attribution. | Planned only; not implemented |
| Video reference scripts | `NUS-VIDEO-001` | editor, reviewer, AI assistant | `AC-VIDEO-001` | `NQA-VIDEO-001` | `BriefVersion`, `MediaAssetVersion` | Brief-version and asset-version paths | No final video generation, editing, asset procurement, automated rights clearance, or video production entities. | Planned only; not implemented |
...
| Human approval | `NUS-APPROVAL-001` | reviewer | `AC-APPROVAL-001`, `AC-APPROVAL-002` | `NQA-APPROVAL-001`, `NQA-APPROVAL-002` | `ReviewTask`, `ApprovalDecision`, `MediaAssetVersion`, `AuditLog` | Review-task and approval-decision paths | AI assistant and readiness must not approve, reject, or bypass human review. | Planned only; not implemented |
...
| Role & permission boundaries | `NUS-PERMISSIONS-001` | owner, admin, reviewer, evidence reviewer, editor, viewer, AI assistant | `AC-PERMISSIONS-001` through `AC-PERMISSIONS-003` | `NQA-PERMISSIONS-001` through `NQA-PERMISSIONS-003` | `WorkspaceMember`, `Role`, `Permission`, `RolePermission`, `AuditLog` | Workspace member, role, permission, audit surfaces | Viewer/editor/AI assistant protected-action bypass must fail. | Planned only; not implemented |
...
| `NUS-CONTENT-001` | editor, reviewer, AI assistant | As an editor, I want to capture ideas and content requirements as draft inputs so human review can evaluate them before approval. | `AC-CONTENT-001`, `AC-CONTENT-002` | `NQA-CONTENT-001`, `NQA-CONTENT-002` | P1 | Planned only; not implemented | AI assistant suggestions remain advisory and cannot change protected fields. |
| `NUS-HASHTAGS-001` | editor, reviewer, AI assistant | As an editor, I want hashtags per selected channel as draft recommendations so content can be reviewed for channel fit. | `AC-HASHTAGS-001` | `NQA-HASHTAGS-001` | P1 | Planned only; not implemented | No reach guarantee, trend ingestion, analytics optimization, or attribution. |
| `NUS-VIDEO-001` | editor, reviewer, AI assistant | As an editor, I want video reference scripts so reviewers can evaluate draft video direction without implying final video production. | `AC-VIDEO-001` | `NQA-VIDEO-001` | P1 | Planned only; not implemented | No final video generation, editing, asset procurement, or automated rights clearance. |
...
AI assistant must not approve, reject, accept evidence, invalidate evidence, publish, schedule, spend, connect accounts, ingest analytics, attribute results, or change protected fields.
