...
| Campaign basics | Campaign paths. | Reuse existing campaign list/create/get/update surfaces. | no | defer | None approved. | Minimal risk as existing APIs are sufficient. | Reuse existing campaign APIs. |
...
| Human approval | Review-task and approval-decision paths. | Reuse existing review and approval operations. | no | defer | None approved. | Approval bypass if readiness is conflated. | Reuse existing approval APIs. |
...
| Manual publishing checklist | Publish job, manual evidence, and audit paths. | Implement as UI-derived logic; defer persisted checklist state. | defer | defer | None approved. | Checklist could be mistaken for publishing action. | Defer persisted checklist endpoint. |
