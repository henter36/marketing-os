...
| Evidence | Evidence is wrong URL/channel/content version. | Must require correction or invalidation. | Evidence Contract. | Wrong proof accepted. | Automated/manual evidence test candidate. | Planning only; not implemented. |
...
| UTM Lite | UTM mismatch against evidence. | Correction or invalidation planning must trigger. | TrackedLink, Evidence Contract. | Wrong destination/reporting. | Automated/manual candidate. | Planning only; not implemented. |
...
| Workspace authority | Body `workspace_id` conflicts with route workspace. | Route/context wins; body workspace is not trusted; must fail. | API Spec, OpenAPI contract. | Tenant boundary bypass. | Automated integration test candidate. | Planning only; not implemented. |
