...
test("missing TEMPLATE_RUNTIME_MODE defaults to in_memory", () => {
  assert.equal(resolveTemplateRuntimeMode({}), "in_memory");
  assert.equal(loadConfig({}).templateRuntimeMode, "in_memory");
});

test("explicit TEMPLATE_RUNTIME_MODE values resolve directly", () => {
  assert.equal(resolveTemplateRuntimeMode({ TEMPLATE_RUNTIME_MODE: "in_memory" }), "in_memory");
  assert.equal(resolveTemplateRuntimeMode({ TEMPLATE_RUNTIME_MODE: "repository" }), "repository");
});

test("empty TEMPLATE_RUNTIME_MODE preserves missing-mode compatibility", () => {
  assert.equal(resolveTemplateRuntimeMode({ TEMPLATE_RUNTIME_MODE: "" }), "in_memory");
});

test("invalid TEMPLATE_RUNTIME_MODE throws a safe configuration error", () => {
  assert.throws(
    () => resolveTemplateRuntimeMode({
    ...
    (error) => {
      assert(error instanceof ConfigurationError);
      assert.equal(error.name, "ConfigurationError");
      assert.equal(error.code, "INVALID_TEMPLATE_RUNTIME_MODE");
      assert.equal(error.message, "Invalid TEMPLATE_RUNTIME_MODE. Allowed values: in_memory, repository.");
      assert.match(error.message, /in_memory/);
      assert.match(error.message, /repository/);
      assert.doesNotMatch(error.message, /bad/);
      assert.doesNotMatch(error.message, /DATABASE_URL|postgres|password|db\.example|user|SECRET_TOKEN|super-secret-token/i);
